// MainSystem.ino
// ESP32 sketch: listens on Serial for the text commands "start" and "stop"
// When "start" is received the relay is activated. When "stop" is received the relay is deactivated.
...
// Note: On ESP32 core versions analogWrite is provided; ensure BTS_LPWM_PIN/BTS_RPWM_PIN are PWM-capable.
...
void setup() {
  ...
  Serial.println("MainSystem ready. Send 'start' to activate relay or 'stop' to deactivate.");
...
// Read an incoming line from Serial if available. Returns true when a full line was read.
// lineBuffer will contain the trimmed line (no CR/LF, leading/trailing spaces removed).
bool readSerialLine(String &lineBuffer) {
  static String line = "";
  while (Serial.available()) {
    ...
    line += c;
    // avoid runaway memory usage: cap the line length
    if (line.length() > 256) {
      line = line.substring(line.length() - 256);
    }
  }
  return false;
}
...
void loop() {
  ...
  if (readSerialLine(cmd)) {
    ...
    if (cmd.startsWith("motor rev") || cmd.startsWith("motor reverse")) {
      ...
      int spaceIdx = cmd.indexOf(' ');
      String rest = (spaceIdx >= 0) ? cmd.substring(spaceIdx + 1) : ""; // 'rev ...'
      rest.trim();
